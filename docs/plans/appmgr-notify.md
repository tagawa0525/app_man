...
| 項目 | 決定 | 判断 |
|------|------|------|
| チャネル | 仕様の 4 実装 (SMTP / TeamsWebhook / File / Multi) + off をすべて標準ライブラリで実装 (net/smtp、net/http、os)。FromConfig は (チャネル名, Notifier) の列を返し (単一モード 1 要素 / multi は展開 / off は空)、runner は宛先 × チャネルごとにレコードを作る (チャネル別レコード方式)。MultiNotifier 型は仕様 §5.9 が列挙するため IF 実装として残すが runner は使わない | 外部依存ゼロ。SMTP は AUTH なし平文 (社内リレー前提、仕様の設定例に認証項目が無い)。同報 1 レコードだと multi 部分成功の再送で成功済みチャネルへ重複送信されるため (PR #37 Copilot 指摘) |
...
| 送信フロー | 通知対象 × チャネルごとに: 重複チェック ((kind, channel, related_entity_type, related_entity_id) に sent があればそのチャネルをスキップ) → notifications へ pending 作成 → Send → 成功: status=sent + sent_at / 失敗: status=failed + last_error | 仕様「送信前に必ずレコード作成」。キーに channel を含め、multi 部分成功の再実行は未達チャネルだけを再作成する |
| --retry-failed | status='failed' かつ retry_count < notification_max_retry (app_settings、既定 5) を、レコードの channel に対応する Notifier で再送。成功: sent / 失敗: retry_count++ (上限到達で gave_up)。同一 (kind, channel, recipient, related) の sent が存在する failed は再送対象から除外し skipped_superseded としてログ | 仕様どおり + 二重送信防止 (PR #37 Copilot 指摘): 重複抑止は sent のみを見るため、failed 残存中に通常実行が同一イベントを再作成・送達した後に旧 failed を再送すると二重になる |
...
- **満了検出クエリの差し替え** (`ListLicensesExpiringInDays` →
  ...
  テストできる副次効果もある
- **PR #37 Copilot 指摘 4 件への対応**: (1) SMTPNotifier は From /
  Recipient / Subject の CR/LF を接続前に拒否 (ヘッダインジェクション対策)。
  (2) notifier.multi.channels の重複を validate で拒否 (同一通知の二重送信
  防止)。(3)(4) チャネル別レコード方式へ変更 — 上記「主要決定」の
  チャネル / 送信フロー / --retry-failed 行を同期済み。status に
  superseded 等の新しい値は増やさず、除外は
  ListFailedNotificationsForRetry の NOT EXISTS で行う
- **PR #37 Copilot 追加指摘 2 件への対応**: (5) gave_up サマリの
  チェックポイント (ListUnsummarizedGaveUp) を **sent のサマリ**の
  MAX(created_at) に限定。失敗サマリは管理者に届いていないため、
  チェックポイントを進めると以前の gave_up が将来のサマリから永久に
  漏れる。失敗サマリは --retry-failed が同一本文で再送するが、翌日の
  通常実行が先に再サマリした後で再送が成功すると同じ行が二重掲載され
  うる — 「漏れ」より許容できると判断 (当日 dedup は作成基準のままで、
  同日の再作成連打を防ぐ役割。配送は retry が担う)。(6) 宛先解決が
  候補者ゼロ (license_manager 不在かつ system_admin も不在) のとき
  warn ログ + skipped_no_recipient に計上し、静かなスキップをなくす
