...
| Login form パッケージ配置 | `internal/view/auth/` (login.templ + login_layout.templ) | 既存 `view/<entity>/` のパターンを踏襲 |
...
| 構造化ログ | 成功: `info "login success" username role app_user_id`、失敗: `warn "login failed" username reason`、ログアウト: `info "logout" app_user_id`。**パスワード平文 / ハッシュは絶対に出さない** | 監査要件 |
...
- sqlc 再生成
- `internal/view/auth/login.templ` + `internal/view/auth/login_layout.templ` (Base を使わない薄い HTML)
