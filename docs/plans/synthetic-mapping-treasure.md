...
- 要件書 § 4 で確定済みの 23 テーブル DDL の SQL 化（6 マイグレーションに分割）
...
make migrate-up
sqlite3 ./data/app.db ".tables"           # 23 テーブル + v_license_usage
...
- 23 テーブル DDL の写経は要件書 `docs/specs/02_要件定義.md` § 4.2 を正本とする。論理削除は **必ず日時カラム**（`*_at` / `valid_to`）で、boolean フラグへの変換は厳禁（CLAUDE.md 規約）
...
- マージコミット例：「Why: 機能実装のための DB レイヤが必要。What: 23 テーブル DDL を 6 マイグレーション化、modernc/sqlite 接続、起動時版数チェック、chi 置換、appmgr-create-app-user 骨格、Makefile 拡張。Impact: 既存 /healthz の挙動は不変。`make migrate-up` 未実行で `appmgr-server` は exit 1 で停止する」
