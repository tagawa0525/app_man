...
| マイグの参照方式 | サーバ側 / `make migrate-up` 経路ともに `embed.FS` 同梱の `internal/db.MigrateUp/Down` を経由する。`db/migrations/embed.go` で `//go:embed *.sql` した `embed.FS` を `package migrations` が export し、`internal/db` が `iofs.New(fs, ".")` で読む。CLI 経路は新規バイナリ `appmgr-migrate` がこのランナを呼ぶ | 当初は nix の `go-migrate` バイナリで外部ファイル直読みを想定していたが、その `migrate` バイナリは内部で snowflake driver を blank import しており `gosnowflake` の init() で「failed to parse CA certificate」の panic を起こして使えない。回避策として自前の `appmgr-migrate` を BINARIES に追加し、内部 Go から `db.MigrateUp/Down` を呼ぶ構成にする。これにより embed.FS が唯一の正本になり 2 重管理も避けられる。 |
