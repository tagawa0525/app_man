...
| lock 排他方式 | OS のファイルロック（`syscall.LOCK_EX\|LOCK_NB`）+ PID 書込 + stale 検出 | `O_CREAT\|O_EXCL` だけだと異常終了時にゴミが残る。flock なら fd クローズで自動解放、stale 判定は PID で補完 |
...
4. `test(lockfile): stale な PID の lock は再取得できる` — RED（`baseDir/<name>.lock` に存在しない PID を書き込んで Acquire が成功することを確認）
5. `feat(lockfile): stale 検出（os.FindProcess + Signal(0)）を実装` — GREEN
...
// stale（PID 不在）な lock は再取得して上書きする。
...
lock ファイルの内容は JSON 1 行：`{"pid":12345,"started_at":"2026-05-21T10:00:00+09:00","binary":"appmgr-backup"}`。stale 判定では PID のみ参照。
...
./bin/appmgr-notify --config config.yml ; echo "exit=$?"          # exit=0（stale を上書き取得）
```
...
- `os.FindProcess` は Unix では常に成功するため、生存判定は `proc.Signal(syscall.Signal(0))` の戻り値で行う（`os.ErrProcessDone` または `errors.Is(err, syscall.ESRCH)` なら stale）
