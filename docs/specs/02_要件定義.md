...
CREATE TABLE devices (
  ...
  hostname TEXT,
  primary_user_id INTEGER REFERENCES users(id),         -- 主利用者（兼務社員が他部署 PC を使う場合の利用実態）
  department_id INTEGER REFERENCES departments(id),     -- 管理責任部署（資産台帳上の所管。primary_user の所属とは独立）
...
**`department_id` と `primary_user_id` の意味分け**：

- `department_id` は端末の **管理責任部署**（資産台帳の所管・棚卸し対象を決める軸）
- `primary_user_id` は端末の **主利用者**（ライセンスの実消費を見る軸）
- 兼務社員が別部署の PC を使うケース（例：営業本部所属の社員が、製造本部所管の検証 PC を使用）はこの 2 軸で表現する
- ライセンス突合は割当のあるユーザ／端末ベースで行うため、専任部署と利用端末がずれていても問題なく評価できる

**兼務社員の扱い**：

- `users.department_id` は **専任部署のみ**を保持する（兼務情報は本システムでは管理しない）
- 兼務に伴う管理権限の付与は `user_department_roles` に複数行を持たせることで表現する（例：営業部の `general_user` 兼 製造部の `license_manager`）
...
CREATE TABLE app_users (
  ...
  notify_email TEXT,                                    -- 通知宛先。NULL なら linked_user.email を使用
...
1. `app_users.notify_email` に値があればそれを使用
2. なければ `linked_user_id` 経由で `users.email` を使用
3. どちらも空のローカル admin に通知が必要な場合、`appmgr-notify` 実行時に warn ログを出して送信スキップ

ローカル admin（`auth_type=local`、`linked_user_id=NULL`）は `notify_email` を実質必須として `appmgr-create-app-user` 側で入力を促す。
...
CREATE TABLE inventory_audits (
  ...
  result_note TEXT,
  snapshot_json TEXT,                                   -- 完了時点の保有本数・割当数・承認状況のスナップショット
  UNIQUE(department_id, fiscal_period)
);

CREATE TABLE audit_logs (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  app_user_id INTEGER REFERENCES app_users(id),         -- 操作者（NULL はバッチ等のシステム操作）
  action TEXT NOT NULL,                                 -- 'create' / 'update' / 'delete' / 'view_sensitive' / 'export' / 'login' / ...
  entity_type TEXT NOT NULL,                            -- 'license' / 'user' / 'app_settings' / ...
  entity_id INTEGER,
  diff_json TEXT,                                       -- 後述のスキーマに従う
...
**`audit_logs.diff_json` のスキーマ**：

```json
{
  "before": { "field_a": "旧値", "field_b": 10 },
  "after":  { "field_a": "新値", "field_b": 12 },
  "context": "任意の補足（例: 'AD 同期による自動更新'）"
}
```

- `create` 操作では `before` を省略、`delete` 操作では `after` を省略
- 差分対象は変更があったフィールドのみ（全フィールド出力はしない）
- 機微情報（`product_keys`、`password_hash`）は値そのものを記録せず `"***"` でマスクし、変更の事実のみを残す

#### notifications

通知の送信履歴・失敗キュー。`appmgr-notify` が再送に使う。

```sql
CREATE TABLE notifications (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  kind TEXT NOT NULL,                                   -- 'expiry' / 'prohibited_detected' / 'unapproved_detected' / 'deactivated_assignment' / 'audit_request' / 'audit_overdue'
  channel TEXT NOT NULL,                                -- 'smtp' / 'teams' / 'file'
  recipient TEXT NOT NULL,                              -- メールアドレスまたは Webhook URL の識別子
  subject TEXT,
  body TEXT,
  related_entity_type TEXT,                             -- 関連するエンティティ（重複送信抑止のキー）
...
- `appmgr-notify` の通常実行：`status='pending'` のレコードを作成して即時送信を試みる
- 送信失敗時は `status='failed'`・`retry_count++`・`last_error` を記録
- `appmgr-notify --retry-failed` で `status='failed'` の再送（`retry_count` が上限を超えたら `gave_up`）
- 上限値は `app_settings.notification_max_retry`（デフォルト 5）
...
```sql
-- 製品ごとの保有・利用状況サマリ。
--   total_owned          : 期限内ライセンスの保有本数合計（満了済みは除外）
--   installed_count      : SKYSEA で検出されたインストール実数（端末単位、未アンインストール）
--   user_assigned_count  : ユーザに対するアクティブな割当数
--   device_assigned_count: 端末に対するアクティブな割当数
-- 過不足判定では user/device の単位が混在しうるため、画面側で count_unit ごとに集計し直すこと。
-- このビューはダッシュボード概要表示用であり、厳密な「実消費数」は別途算出する。
...
- `expires_at < now()` のライセンスは保有本数の集計（`v_license_usage.total_owned`）から自動的に除外される
- レコード自体は削除せず、過去の契約履歴として残す（証書ファイル・`meta.yml` も保持）
- 一覧画面では「満了」フィルタで表示／非表示を切替（デフォルトは現役のみ）
- 「ライセンス超過」「ライセンス不足」のアラート判定も期限内ライセンスのみで行う

**契約更新（renewal）**：

- 継続契約は **新ライセンスを別レコードで作成**する（旧レコードは満了させてそのまま残す）
- 旧契約との関連は `licenses.note` に「2024 年度契約から継続更新（旧 license_id=123）」のようにテキストで残す
- DB レベルの親子関係は持たせない（運用負荷に対して利得が小さい）
...
棚卸し完了時、`inventory_audits.snapshot_json` にその時点の確認内容を保存する。内部統制の証跡として、後から「何を確認した状態で完了とした」が復元できる必要があるため。

スキーマ：

```json
{
  "completed_at": "2026-03-31T15:00:00+09:00",
  "licenses": [
    { "license_id": 12, "product": "Adobe Acrobat Pro DC", "total": 10, "user_assigned": 8, "device_assigned": 0 }
  ],
  "approvals": [
    { "product_id": 5, "product": "Slack", "status": "approved" }
  ],
  "discrepancies": "営業 1 課で 1 件、所在不明の割当あり（次回までに精査）"
}
```

- 確認の単位は「ライセンス1件ずつのチェック」ではなく、画面に表示された一覧をまとめて確認する形式
- `discrepancies` は管理者が任意で書き込む補足（残課題・備考）
...
通知は送信前に必ず `notifications` テーブルにレコードを作成し、送信成否を記録する（4.2 参照）。送信失敗時の動作：

- `status='failed'` で記録、`last_error` にエラー詳細を残す
- `appmgr-notify --retry-failed` で `status='failed'` かつ `retry_count < notification_max_retry` のレコードを再送
- `retry_count` が上限を超えたら `status='gave_up'` に遷移し、system_admin 向けの日次サマリに集約
- 同一イベントの重複送信抑止は `(kind, related_entity_type, related_entity_id)` をキーに `sent` レコードがあれば再作成しない

**宛先解決の優先順位**は `app_users.notify_email` → `linked_user.email` の順（4.2 app_users 参照）。両方空の宛先は `appmgr-notify` 実行時に warn ログを出してスキップする。
...
| キー                                 | 用途                                              | 既定値 |
| ------------------------------------ | ------------------------------------------------- | ------ |
| `notification_max_retry`             | 通知再送の上限回数                                | 5      |
| `retention_days_audit_logs`          | `audit_logs` の保持期間（日）                     | 1825   |
| `retention_days_raw_installations`   | `raw_installations` の保持期間（日）              | 365    |
| `retention_days_import_logs`         | `import_logs` の保持期間（日）                    | 1095   |
| `retention_days_notifications_sent`  | `notifications`（送信済み）の保持期間（日）       | 365    |

保持期間を過ぎたレコードは `appmgr-prune-logs` バイナリで物理削除する（9 節参照）。`audit_logs` は会計監査・内部統制の対象になり得るため既定 5 年と長めに取る。
...
- **候補の自動提示**：`raw_product_name` に対して `products.canonical_name` および `product_aliases.alias_name` との部分一致（大文字小文字無視・空白正規化）で候補を最大 5 件表示
- **ベンダー別グルーピング**：`raw_vendor_name` で集約表示し、同一ベンダーの未解決製品をまとめて処理できる UI
- **エイリアスの自動学習**：紐付け確定時に `raw_product_name` を `product_aliases` に自動登録する（チェックボックスで抑止可）。以後、同一文字列は SKYSEA 取込み時点で自動解決される
- **一括操作**：複数行を選択して同一製品への紐付け／一括無視

初期取込み時のような大量データには `appmgr-import-bootstrap`（9 節）で機械的に処理し、運用フェーズの少数件は本画面で対応する。
...
- ログイン成功時は **セッション ID を再発行**する（セッション固定攻撃対策）
- ローカル admin のパスワードリセットは `appmgr-create-app-user --reset-password --username <name>` で行う（実行時に新パスワードをプロンプト入力、`bcrypt` で再ハッシュして `password_hash` を上書き）。アプリ起動中でも DB を直接更新する形で動作する
- 認証層は抽象化：
...
- **ログイン成功時にセッション ID を再発行**（セッション固定攻撃対策）
- **CSRF 対策**：GET 以外のリクエストには CSRF トークンを必須とする。`templ` の共通レイアウトで hidden input を自動挿入し、HTMX の `hx-headers` または `meta` タグ経由で送信する。サーバ側はセッションごとに発行したトークンと突合
- ファイルダウンロードは認可チェック後にストリーミング配信
- アップロードファイルのバリデーション：
  - サイズ上限（`file_store.upload_max_bytes`）
  - 許可拡張子リストとの照合
  - **マジックバイト（実体ヘッダ）判定**で `application/pdf` / `image/png` / `image/jpeg` を実検証する（`Content-Type` ヘッダは信用しない）
...
CLI バイナリ（`appmgr-import-skysea` / `appmgr-sync-directory` / `appmgr-check-integrity` / `appmgr-notify` / `appmgr-backup` / `appmgr-prune-logs` / `appmgr-generate-meta` / `appmgr-import-bootstrap`）は同時実行されると SQLite WAL での競合や `VACUUM INTO` 中の書込み衝突を起こしうる。タスクスケジューラの設定ミス・手動実行と日次起動の重なりに備えて、共通基盤で排他制御を行う：

- 各バイナリは起動時に `<base>/locks/<binary-name>.lock` を排他オープン（`O_CREAT|O_EXCL` 相当 or OS のファイルロック API）し、PID と起動時刻を書き込む
- 取得失敗時は exit code 2 で即終了し、ログに「他プロセス実行中」と記録（タスクスケジューラ側でエラー扱いとなる）
- 正常終了時に lock ファイルを削除する。異常終了で残った場合は PID の生存確認で stale なら自動破棄
- `appmgr-backup` だけは `appmgr-*` の他全バイナリと相互排他とする（VACUUM が他の書込みと衝突するため、グローバルロック相当）
- `appmgr-server` 本体は常駐前提のため、サーバ側は別の lock ファイル（`appmgr-server.lock`）で多重起動だけ防ぐ。バッチ系とは別管理
...
| `appmgr-notify`           | 日次通知メール送信（満了アラート等）／失敗分の再送      | 日次             |
| `appmgr-backup`           | DB バックアップ                                         | 日次             |
| `appmgr-prune-logs`       | 保持期間超過レコードの物理削除（5.11 のキーを参照）     | 日次／週次       |
| `appmgr-generate-meta`    | meta.yml 一括再生成                                     | 必要時           |
| `appmgr-create-app-user`  | 初期管理者ユーザ登録／パスワードリセット（ローカル認証）| 初期セットアップ |
| `appmgr-import-bootstrap` | 既存 Excel/CSV からの初期データ一括投入                 | 初期セットアップ |
...
appmgr-create-app-user --config config.yml --reset-password --username admin
appmgr-notify --config config.yml --kind expiry
appmgr-notify --config config.yml --retry-failed
appmgr-prune-logs --config config.yml
appmgr-prune-logs --config config.yml --dry-run
appmgr-import-bootstrap --config config.yml --kind products   --file ./bootstrap/products.csv
appmgr-import-bootstrap --config config.yml --kind licenses   --file ./bootstrap/licenses.csv
appmgr-import-bootstrap --config config.yml --kind assignments --file ./bootstrap/assignments.csv
```

**`appmgr-import-bootstrap` の仕様**：

導入初日に既存の各部署 Excel／部門台帳を取り込むためのワンショット運用バイナリ。MVP に必須。

- 入力は UTF-8 CSV（テンプレートをリポジトリの `docs/templates/` に同梱）
- `--kind` で投入対象を切替：`vendors` / `products` / `product_aliases` / `licenses` / `users` / `devices` / `assignments`（user/device 自動判別）
- 投入前に **検証フェーズ**を必ず実行：存在しない外部キー参照・必須列欠落・重複キーを行番号付きで標準出力に列挙
- `--dry-run` で検証のみ実施、`--commit` で実投入（既定は dry-run）
- 投入は 1 トランザクションで実施し、エラー時は全件ロールバック
- 投入結果は `audit_logs` に `action='bootstrap_import'` で記録
- 名寄せキューに登録された大量の `raw_installations` に対しては、CSV で「raw_product_name → product_id」のマッピングを与えて一括解決するモード（`--kind alias-resolve`）も提供する
...
```yaml
server:
  listen: 0.0.0.0:8180                    # 社内 LAN からのアクセスを受ける。8080 は既存利用中のため避ける
  base_url: http://appmgr.example.local:8180
...
locks:
  base_dir: ./data/locks              # CLI バイナリ間の排他制御用 lock ファイル（8.8 参照）
...
15. **`appmgr-import-bootstrap` でテンプレート CSV を投入すると、製品マスタ・ライセンス・割当が一括登録され、`audit_logs` に `bootstrap_import` として記録される**。`--dry-run` で検証のみ実行できる。
16. **`appmgr-notify` の SMTP 送信を意図的に失敗させた状態で実行すると `notifications.status='failed'` となり、`--retry-failed` での再送で `status='sent'` に遷移する**。
17. **`appmgr-prune-logs` を実行すると、`app_settings` の保持期間キーに従って `audit_logs` / `raw_installations` / `import_logs` / `notifications`（送信済み）の古いレコードが削除される**。`--dry-run` で対象件数のみ確認できる。
18. **`appmgr-import-skysea` 実行中に `appmgr-backup` を起動するとロック取得に失敗し、exit code 2 で終了する**（バッチの排他制御）。
19. **ローカル admin のパスワードを `appmgr-create-app-user --reset-password` で再設定すると、再設定後のパスワードでログインできる**。
20. **棚卸しを完了すると、`inventory_audits.snapshot_json` に確認時点の集計値が保存される**。
...
1. **基盤整備**：プロジェクト構造（`cmd/` のディレクトリ分割を最初から）、設定読込、ロギング、DB 接続、マイグレーション、CLI 共通の lock ファイル基盤
2. **マスタ系**：vendors、products、product_aliases、departments、users、devices の CRUD と画面
3. **認証・認可**：app_users（notify_email 含む）、ローカルログイン、CSRF・セッション ID 再発行、user_department_roles、ミドルウェア、`appmgr-create-app-user`（パスワードリセット含む）
4. **AD 連携（CSV）**：`appmgr-sync-directory`、CSVSource 実装、退職者検出、`general_user` 自動付与
5. **一般社員向け画面**：`/my/licenses`、`/search`、ダッシュボードの権限分け
6. **ライセンス・証書**：licenses、license_documents、ファイル保存（マジックバイト検証）、meta.yml 生成
7. **初期データ移行**：`appmgr-import-bootstrap`、テンプレート CSV と検証ロジック
8. **SKYSEA 取込み**：`appmgr-import-skysea`、raw_installations、名寄せキュー（候補提示・エイリアス自動学習）
9. **承認管理**：default_approval_status、department_product_approvals、評価ロジック、画面
10. **棚卸し**：inventory_audits、画面、スナップショット、通知連携
11. **通知**：`appmgr-notify`、Notifier 抽象化、notifications テーブル、再送、SMTP/Teams Webhook/ファイル
12. **エクスポート・整合性チェック・バックアップ・保持期間**：`appmgr-check-integrity`、`appmgr-backup`、`appmgr-generate-meta`、`appmgr-prune-logs`
13. **部署改廃 UI**：後継部署設定、ライセンス移管
14. **AD 連携（LDAP）**：LDAPSource 実装、LDAPAuthenticator
15. **仕上げ**：監査ログの網羅、画面の細部、README、タスクスケジューラ登録例
